...
SLACK_WEBHOOK  = os.getenv("SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/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")
# 이벤트 타입별 SNS Topic ARN 매핑
SNS_TOPIC_MAP = {
...
# ── [수정 #2] 실제 클라이언트 IP 추출 ──────────────────────────────────────
# ALB 환경: X-Forwarded-For 마지막 값 = ALB가 추가한 신뢰할 수 있는 실제 IP
# 이전: .split(",")[0] → 공격자가 첫 번째 값을 위조 가능
def get_client_ip():
    xff = request.headers.get("X-Forwarded-For", "")
    if xff:
        return xff.split(",")[-1].strip()
    return request.remote_addr
# ── [추가 #1] SNS publish — Slack 알람에 IP/username 포함 ──────────────────
# CloudWatch 알람 → SNS → Lambda 경로는 통계 정보만 전달되어 UNKNOWN 표시됨.
# app.py가 이벤트 감지 시점에 직접 SNS에 컨텍스트를 담아 publish하면
# Lambda가 파싱해서 Slack에 실제 IP/username을 표시할 수 있음.
...
def check_geoip_block(ip, username):
    try:
        ...
        if not os.path.exists(mmdb):
            print(f"[GeoIP] mmdb 파일 없음: {mmdb}", flush=True)
            return False
...
def login_required(view_func):
    ...
    def wrapped(*args, **kwargs):
        ...
        if session.get("login_ip") and session.get("login_ip") != current_ip:
            ...
            # ★ SNS publish — Lambda가 실제 IP/username을 Slack에 표시 가능
            publish_alert(
...
def download():
    ...
    if not (is_admin or f["owner_id"] == user_id or user_level in allowed):
        ...
        # ★ SNS publish — IDOR 공격자 IP/username Slack에 표시
        publish_alert(
            "IDOR_DETECTED",
            username=username,
            ip=ip,
...
def login():
    if request.method == "POST":
        ...
        if blocked_country:
            ...
            # ★ SNS publish — 해외 로그인 공격자 IP/국가 Slack에 표시
            publish_alert(
                "ABNORMAL_LOCATION_LOGIN",
                username=u,
                ip=ip,
                extra={"country": blocked_country}
            )
            send_slack_alert(f"🚨 해외 로그인 탐지\n사용자: {u}\n국가: {blocked_country}\nIP: {ip}")
        ...
        if lock_row:
            ...
            if new_count >= 10:
                ...
                db.commit()
                # ★ SNS publish — Brute-force 공격자 IP/타깃 계정 Slack에 표시
                publish_alert(
                    "BRUTE_FORCE_DETECTED",
                    username=u,
                    ip=ip,
                    extra={"fail_count": new_count}
                )
